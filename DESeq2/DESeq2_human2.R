...
spec = matrix(c(
    ...
    'l_repetitions'   , 'l', 1, "integer",'treatment-bio-repetitions',
    'r_repetitions'   , 'r', 1, "integer",'control-bio-repetitions',
...
par(ask=T)
##################需要修改的 ###########
#[0] 分组信息condition <- factor(c(rep("Cre",3),rep("WT",3))) #condition <- factor(c(rep("Cre",3),rep("WT",3)),levels = c("Cre","WT"))
#[1] 分组最重要的地方，有时候上面分组会混乱，这里一锤定音 res <- results(dds1,contrast = c('condition', 'Cre', 'WT'))
#[2] 物种数据库 org.Hs.eg.db替换org.Hs.eg.db  # 只能对应两个字母，并且第一个字母大写
...
#res <- results(dds1,contrast = c('condition', 'A', 'B'))
#summary(res) 
...
# 查看特定基因的信息
# library(dplyr)
#select(org.Hs.eg.db, keys = "CW07G09620", columns = c("GO"))

#pathway2gene <- AnnotationDbi::select(org.Hs.eg.db, 
...
# ID 转换
ids<- bitr(rownames(res1_total), fromType = "SYMBOL",toType = c( "ENTREZID"),OrgDb = org.Hs.eg.db ,drop = T)

Up_ids<- bitr(rownames(res1_up), fromType = "SYMBOL",toType = c( "ENTREZID"),OrgDb = org.Hs.eg.db ,drop = T)
Down_ids<- bitr(rownames(res1_down), fromType = "SYMBOL",toType = c( "ENTREZID"),OrgDb = org.Hs.eg.db ,drop = T)
...
# 绘图
goBP_sorted <- goBP[order(goBP$pvalue), ]
goCC_sorted <- goCC[order(goCC$pvalue), ]
goMF_sorted <- goMF[order(goMF$pvalue), ]

# 提取每个类别的前 10 行结果（或自定义行数）
n <- 10  # 每个类别取前 10 行

n_bp <- min(n, nrow(goBP_sorted))
n_cc <- min(n, nrow(goCC_sorted))
n_mf <- min(n, nrow(goMF_sorted))

go_enrich_df <- data.frame(
  ID = c(head(goBP_sorted$ID, n_bp), head(goCC_sorted$ID, n_cc), head(goMF_sorted$ID, n_mf)),
  Description = c(head(goBP_sorted$Description, n_bp), head(goCC_sorted$Description, n_cc), head(goMF_sorted$Description, n_mf)),
  GeneNumber = c(head(goBP_sorted$Count, n_bp), head(goCC_sorted$Count, n_cc), head(goMF_sorted$Count, n_mf)),
  type = factor(
    c(rep("biological process", n_bp), 
      rep("cellular component", n_cc),
      rep("molecular function", n_mf)),
    levels = c("molecular function", "cellular component", "biological process")
  )
)

# go_enrich_df<-data.frame(ID=c(goBP$ID, goCC$ID, goMF$ID),
#                          Description=c(goBP$Description, goCC$Description, goMF$Description),
#                          GeneNumber=c(goBP$Count, goCC$Count, goMF$Count),
#                          type=factor(
...
kegg_enrich <- enrichKEGG( ids$ENTREZID,
              organism   = "hsa", #需要小写，https://www.genome.jp/kegg/catalog/org_list.html
              pvalueCutoff = 0.05,
              qvalueCutoff = 0.2,
              minGSSize = 10, #  默认值，最小的基因富集数量
              )
